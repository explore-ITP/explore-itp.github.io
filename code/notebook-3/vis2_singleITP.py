...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 24 14:11:24 2020
...
# Creates filelist, where rawlocs.dat files will be opened and analyzed
filelist = []
...
# ALL DATA
# Iterates through rawlocs.dat files and reads data
...
trace = go.Heatmap(
   ...
   type = 'heatmap',
   colorscale = "RdYlBu_r",
   hovertemplate='Lat: %{x}<br>Lon: %{y}<br>No. Obs: %{z}<extra></extra>',
   colorbar=dict(title='Number of Observations') 
)
data = [trace]

# Add axis titles
layout=go.Layout(
        title="Location Density of All ITP's",
        xaxis=dict(title='Longitude (to nearest degree)'),
        yaxis=dict(title='Latitude (to nearest degree)')
    )
...
iplot(fig)
 ...
 # Change colormap of plot by setting one of the following equal to 'cmap': [‘Accent’, ‘Accent_r’, ‘Blues’, ‘Blues_r’, ‘BrBG’, ‘BrBG_r’, ‘BuGn’, ‘BuGn_r’, ‘BuPu’, ‘BuPu_r’, 
